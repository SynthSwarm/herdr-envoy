...
test("coordinator initialises recovery, provisions guidance and exposes only coordinator tools", async (t) => {
  ...
  const request = t.mock.method(Coordinator.prototype, "requestAgent", async () => "queued");
  const cancel = t.mock.method(Coordinator.prototype, "cancelRequest", async () => "retired");
  ...
  assert.equal(await hooks.tool.cancel_request.execute({ jobId: "request" }, { sessionID: "owner" }), "retired");
  assert.deepEqual(cancel.mock.calls[0].arguments, ["request", "owner"]);
...
test("delegate consumes the handoff, recovers its task on reload and reports using consumed auth", async (t) => {
  ...
  assert.deepEqual(Object.keys(hooks.tool).sort(), ["ask", "complete", "hand_back", "read_task"]);
